...
#set mongo connection parameters
...
def lockmongo():
	bool_locked = mongo_conn.fsync(lock = True)
	if bool_locked == True:
		logger.info('Attempted to lock. Mongo locked.')
	else:
		logger.info('Attempted to unlock. Mongo is unlocked')

def unlockmongo():
	bool_locked = mongo_conn.unlock()
	if bool_locked == True:
                logger.info('Attempted to unlock. Mongo locked.')
        else:
                logger.info('Attempted to unlock. Mongo is unlocked')
